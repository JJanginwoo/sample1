...
public class TestController {
	...
	/*
	 * - 스프링에서는 파일을 받기 위한 스펙을 제공하고 있으며 그중 하나가 MultipartFile 클래스에 바로 바이너리 파일을 넣는 형태를
	 * 제공한다. - 형식 : @RequestParam("type="file" input 이름") MultipartFile file
	 */
	@PostMapping("/test1")
	...
	@PostMapping("/test3")
	public String multiFileUpload(@RequestParam("mediaFile") MultipartFile[] files, @RequestParam String user,
			@RequestParam String url, Model model) throws IllegalStateException, IOException {
		for (MultipartFile file : files) {
			if (!file.getOriginalFilename().isEmpty()) {
				Path path = Paths.get("C:/sample/" + file.getOriginalFilename());
				file.transferTo(path);
	...
	public String multiFileUpload(MediaVO vo) throws IllegalStateException, IOException {
		...
		for (MultipartFile file : files) {
			if (!file.getOriginalFilename().isEmpty()) {
				Path path = Paths.get("C:/sample/" + file.getOriginalFilename());
				file.transferTo(path);
				System.out.println(file.getOriginalFilename() + "저장이 되었습니다.");
			} else {
				System.out.println("에러가 발생헀습니다.");
			}
		}
